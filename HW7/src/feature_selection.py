...
def correlation_selection(X, y):
    ...
    return corr.to_frame("correlation")

def mutual_information_selection(X, y, random_state=42):
    mi = mutual_info_regression(X, y, random_state=random_state)
    return pd.DataFrame({"feature": X.columns, "mutual_information": mi}).sort_values("mutual_information", ascending=False)
...
def feature_selection_voting(X, y, top_k=6):
    ...
    return votes.sort_values("total_votes", ascending=False), {
        "correlation": corr,
